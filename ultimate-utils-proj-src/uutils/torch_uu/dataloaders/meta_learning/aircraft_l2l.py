...
#from uutils.torch_uu.dataloaders.aircraft import get_transform


def get_remaining_transforms_aircraft(dataset: MetaDataset, ways: int, samples: int) -> list[TaskTransform]:
...
def get_aircraft_datasets(
    ...
    train_dataset = l2l.vision.datasets.FGVCAircraft(root=root,
                                                transform=train_data_transforms,
                                                mode='train',
                                                download=True)
    valid_dataset = l2l.vision.datasets.FGVCAircraft(root=root,
                                                transform=test_data_transforms,
                                                mode='validation',
                                                download=True)
    test_dataset = l2l.vision.datasets.FGVCAircraft(root=root,
                                               transform=test_data_transforms,
                                               mode='test',
                                               download=True)
    ...
    train_dataset.name = 'train_aircraft'
    valid_dataset.name = 'val_aircraft'
    test_dataset.name = 'test_aircraft'

    _datasets = (train_dataset, valid_dataset, test_dataset)
    return _datasets


class Task_transform_aircraft:
